...
bool TopAcademicAffairsStaff::changePassword(std::string current_pass, std::string new_pass) {
...
bool DepartmentAcademicAffairsStaff::changePassword(std::string current_pass, std::string new_pass) {
	if (checkPassword(current_pass)) {
		setPassword(new_pass);
		return true;
	}
