...
bool MyTerm::haveCourse(Course* course) const {
	for (const auto& c : courses) {
		if (c.first->haveSameCourseID(course)) {
			return true;
		}
	}
	return false;
...
bool MyTerm::haveCourseWithStatus(Course* course, char _status) const {
	for (const auto& c : courses) {
		if (c.first->haveSameCourseID(course) && c.second.getStatus() == _status) {
			return true;
		}
	}
	return false;
...
std::vector<std::unique_ptr<EnrollmentError>> Student::checkEnrollment(int term_no, const std::map<PresentedCourse*, char>& _courses)
{
	...
	for (const auto& course : courses)
	{
		...
		for (const auto& coR : course.first->getCorequisites()) {
			if (coR->searchSameCourseID(keys) == false && this->haveCourseWithStatus(coR, MyCourse::passed) == false) {
		...
		for (const auto& c : courses) {
			if (c != course) {
				if (c.first->haveSameCourseID(course.first)) {
...
bool Student::changePassword(std::string current_pass, std::string new_pass) {
